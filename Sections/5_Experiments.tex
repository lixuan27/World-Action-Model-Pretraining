...
The benchmark tables retain source-reported baselines. Tables~\ref{tab:libero_plus}--\ref{tab:vlabench}
label optimistic JAM estimates; remaining blue rows await evaluation. Paired comparisons fix
demonstrations, adaptation budgets, and environment resets.
Success measures closed-loop control; frozen-feature readouts and input sensitivity assess
representation content. Controlled pretraining and scaling displays retain empty result fields
until measurements are available. Appendix~\ref{app:controls} specifies probe sampling and uncertainty.
...
\begin{figure}[H]
\centering\includegraphics[width=\linewidth]{Figures/scaling_pending.pdf}
\caption{\textbf{How are data and compute effects separated?} LIBERO-Plus comparisons vary unique data, duration, or agent width at otherwise fixed budgets. Axes retain the experimental settings; result areas remain empty until measurement. The full-corpus reference uses 60,000 updates.}
\label{fig:scaling}
\end{figure}
\FloatBarrier

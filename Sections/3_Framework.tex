...
\FOR{$k = 0$ \TO $K' - 1$}
  \STATE $(\sigma_v, \sigma_a) \leftarrow \mathcal{S}[k]$; \;
         $(\sigma_v', \sigma_a') \leftarrow \mathcal{S}[k+1]$
  \IF{$k \geq 2$ \AND $j < m$ \AND
      $\cos\big(\hat{v}^{(k-1)}, \hat{v}^{(k-2)}\big) > \tau$}
    \STATE $\hat{v}^{(k)} \leftarrow \hat{v}^{(k-1)}$; \;
           run the action branch alone to obtain $\hat{a}^{(k)}$; \;
           $j \leftarrow j + 1$
  \ELSE
    \STATE $\big(\hat{v}^{(k)}, \hat{a}^{(k)}\big) \leftarrow
            F_\theta\big(z, a, z^{\text{cond}}, c, s, \sigma_v, \sigma_a; M\big)$; \;
           $j \leftarrow 0$
  \ENDIF
  \STATE $z \leftarrow z + (\sigma_v' - \sigma_v)\, \hat{v}^{(k)}$; \;
...
$\mathtt{vanilla}$ keeps the shared capacity as it stands. The variant $\mathtt{moe}$ adds a
...
% legend
\node[vb, label={[font=\scriptsize, label distance=-1mm]right:{\;video expert}}] at (0.1,-9.0) {};
\node[ab, label={[font=\scriptsize, label distance=-1mm]right:{\;action expert}}] at (3.4,-9.0) {};
\node[ub, label={[font=\scriptsize, label distance=-1mm]right:{\;understanding expert}}] at (7.0,-9.0) {};
\node[cb, label={[font=\scriptsize, label distance=-1mm]right:{\;conditioning branch}}] at (11.2,-9.0) {};
\end{tikzpicture}}
...
$M_{VV}$ takes three values. The mode $\mathtt{bidirectional}$ sets every entry
to one. The mode $\mathtt{per\text{-}frame\ causal}$ builds a lower-triangular
matrix at frame granularity and expands it by the token count of one frame, so a
frame reads its own past. The mode $\mathtt{first\text{-}frame\ causal}$ sets
...
\newcommand{\onst}{fill=tianzhegreen!45, draw=black!35, line width=0.15pt}
\newcommand{\offst}{fill=black!6, draw=black!25, line width=0.15pt}
\newcommand{\halfst}{fill=tianzheblue!32, draw=black!35, line width=0.15pt}
...
\node[anchor=north west, align=left, font=\scriptsize] at (112,-24)
  {rows are queries, columns are keys\\[1pt]
   \textcolor{tianzhegreen!45}{$\blacksquare$}\; readable\\
   \textcolor{tianzheblue!32}{$\blacksquare$}\; readable outside the\\
   \hspace{3mm} conditioning rows\\
   \textcolor{black!12}{$\blacksquare$}\; withheld};
\end{tikzpicture}}
